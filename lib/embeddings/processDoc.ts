// Import OpenAI but we'll use Gemini instead
...
async function createEmbeddingsBatch(
  ...
  // Import the Gemini embeddings utility
  const { createEmbeddings } = await import("@/lib/gemini");
  ...
  for (let i = 0; i < chunks.length; i += batchSize) {
    ...
    // Use Gemini's embedding API instead of OpenAI
